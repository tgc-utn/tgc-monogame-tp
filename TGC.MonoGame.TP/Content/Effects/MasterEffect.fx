...
struct BasicColorVSI
{
    float4 Position : POSITION0;
};

struct BasicColorVSO
{
    float4 Position : SV_POSITION;
};

BasicColorVSO BasicColorVS(in BasicColorVSI input)
{
    BasicColorVSO output;
    output.Position = mul(input.Position, WorldViewProjection);
    return output;
}

float4 BasicColorPS(BasicColorVSO input) : COLOR
{
...
{
    float depth = (input.ScreenSpacePosition.z * zMul) / (input.ScreenSpacePosition.w);
    float correctedDepth = depth * depthMul - wMul;
    return float4(correctedDepth, correctedDepth, correctedDepth, 1.0);
}



ShadowedVertexShaderOutput ShadowedVS(in ShadowedVertexShaderInput input)
{
