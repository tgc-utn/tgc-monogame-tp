...
class CylinderGizmoGeometry : RadialGizmoGeometry
{
    ...
    public CylinderGizmoGeometry(GraphicsDevice device, int subdivisions) : base(device)
    {
        ...
        Array.Copy(originalIndices.Select(index => (ushort)(index + subdivisions)).ToArray(), 0, indices,
            subdivisionsTimesTwo, subdivisionsTimesTwo);
        Array.Copy(originalIndices.Select(index => (ushort)(index + subdivisionsTimesTwo)).ToArray(), 0, indices,
            subdivisionsTimesTwo * 2, subdivisionsTimesTwo);
