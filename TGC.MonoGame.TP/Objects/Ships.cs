...
namespace TGC.MonoGame.TP.Objects 
{
    public class Ship 
    {
        ...
        public Matrix UpdateShipRegardingWaves (float time) {
            var newY = (MathF.Sin(Position.X + time) + MathF.Sin(Position.Z + time))*15 + MathF.Sin(Position.X + Position.Z + time)*15 + 50;
            
            var tangent1 = Vector3.Normalize(new Vector3(0, 
                (MathF.Cos(Position.X+time)*15f+   MathF.Cos(Position.X+ Position.Z + time)*15f) * 0.005f
                ,1));
            var tangent2 = Vector3.Normalize(new Vector3(1, 
                (MathF.Cos(Position.Z+time)*15f+   MathF.Cos(Position.X+ Position.Z + time)*15f) * 0.005f
                ,0));
